...
@pytest.mark.parametrize(
    ...
    [
        ...
        # 郵便番号の中の空白（OCR がハイフンを落とした／ハイフンの周りに空白を挟んだ）。
        # 1 回で剥がれないと「1000001東京都…」が残って 2 回目で初めて剥がれる＝不動点でなくなる
        ("〒100 0001 東京都千代田区千代田1-1", "東京都千代田区千代田1-1"),
        ("〒 100 - 0001 東京都千代田区千代田1-1", "東京都千代田区千代田1-1"),
        # 末尾の郵便番号（空白か 〒 で切れているもの）
        ("東京都品川区北品川5-10-20 〒141-0001", "東京都品川区北品川5-10-20"),
        ("東京都品川区北品川5-10-20 141-0001", "東京都品川区北品川5-10-20"),
        ("東京都千代田区千代田1-1 〒100 0001", "東京都千代田区千代田1-1"),
        # 〒 だけ（OCR が数字を落とした）
        ...
        ("本社仙台市泉区紫山3-1-4", "仙台市泉区紫山3-1-4"),  # 区切りなし＋市名
        ("本社愛知県豊田市トヨタ町1", "愛知県豊田市トヨタ町1"),  # 区切りなし＋県名
        ...
        # 空白は、両隣が半角英数字のものだけ 1 つ残し、それ以外は除く（建物名の前の空白も）
        ("東京都新宿区四谷9-9-9 ファッションビル99F", "東京都新宿区四谷9-9-9ファッションビル99F"),
        ("東京都 品川区 北品川 5-10-20", "東京都品川区北品川5-10-20"),
        # 番地の直後の階数／部屋番号、英字の住所。無条件に除くと「1-1-13F」「A-123Sample.BLD」
        # という**別の住所**になる（string 型の頃は 1 つの空白に畳んでいた）
        ("東京都千代田区丸の内1-1-1 3F", "東京都千代田区丸の内1-1-1 3F"),
        ("大阪市中央区本町2-3-4 201号室", "大阪市中央区本町2-3-4 201号室"),
        ("東京都千代田区丸の内1-1-1　　3F", "東京都千代田区丸の内1-1-1 3F"),
        ("A-123 Sample.BLD,1-2-3,sample-city", "A-123 Sample.BLD,1-2-3,sample-city"),
    ],
)
def test_address_jp_rules(raw: str, expected: str) -> None:
    ...
    _assert_fixed_point(expected)


def _assert_fixed_point(value: str) -> None:
    """正規化した値はもう一度通しても変わらない。

    正解データの検査（golden/scripts/check_gold_addresses.py）と計測の採点
    （region_ab.score_key はプロダクトの value_normalized にもう一度通す）の前提。
    """
    assert normalize(FieldType.ADDRESS_JP, value).value == value


@pytest.mark.parametrize(
    ("raw", "expected"),
    [
        # 見出し語で始まる**長い複合語**は見出し語ではない。先頭 2〜3 文字だけ剥がして
        # 「工場愛知県…」「名:東京都…」を作ると、見出し語付きのままより悪い
        ("本社工場 愛知県豊田市トヨタ町1", "本社工場愛知県豊田市トヨタ町1"),
        ("本社ビル 東京都港区1-2-3", "本社ビル東京都港区1-2-3"),
        ("支店名: 東京都港区1-2-3", "支店名:東京都港区1-2-3"),
        ("住所地 東京都港区1-2-3", "住所地東京都港区1-2-3"),
        ("事業所名 東京都港区1-2-3", "事業所名東京都港区1-2-3"),
        ("Addressee 東京都港区1-2-3", "Addressee東京都港区1-2-3"),
    ],
)
def test_address_jp_compound_label_is_not_stripped(raw: str, expected: str) -> None:
    assert normalize(FieldType.ADDRESS_JP, raw).value == expected
    _assert_fixed_point(expected)
