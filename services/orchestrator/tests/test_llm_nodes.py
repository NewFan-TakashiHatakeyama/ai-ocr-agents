...
def test_placeholder_example_記入例語は落とす(text: str) -> None:
    assert region_hint.is_placeholder_example(text) is True
...
def test_placeholder_example_値は落とさない(text: str | None) -> None:
    assert region_hint.is_placeholder_example(text) is False
...
def test_placeholder_example_正解データの値は_sample13_の3項目だけが記入例語() -> None:
    ...
    flagged = sorted(
        ...
        if isinstance(value, str) and region_hint.is_placeholder_example(value)
    )
    assert flagged == [
...
def test_placeholder_example_第4回の例示値の全件で_sample13_由来だけが落ちる() -> None:
    ...
    assert len(items) >= 90
    flagged = sorted((i["doc_type"], i["field"]) for i in items if region_hint.is_placeholder_example(i["example_value"]))
    assert flagged == [
