...
def test_kind_conflict_unknown_は落とさない(monkeypatch) -> None:
    ...
    assert report.dropped == {}
    # 例示値が unknown でも落とさない（「ご担当」は見出し語だけなので placeholder_example に
    # なる。種類が unknown で値でもある語を使う）
    schema2 = _field_schema("customer_name", "string", "営業一課")
...
# ---------------- placeholder_example（記入例語の例示値） ----------------
#
# 判定そのもの（記入例語の一覧・値の一覧・正解データと第 4 回の例示値 94 件の回帰）の
# テストは、判定の移設に合わせて packages/schemas/tests/test_placeholder_example.py に
# 移した。ここには「落ちてヒントが渡らない」経路のテストだけを残す。


def test_placeholder_example_は_newfan_schemas_の判定をそのまま使う() -> None:
    """gateway（保存時の警告）と orchestrator（事前ガード）が同じ判定を使うことの固定。
    region_hint で別実装に戻す（コピーする）と、画面の警告と実行時の判定がずれる。
    ``region_hint.is_placeholder_example`` は既存の参照のために re-export で残す。"""
    import newfan_schemas

    from newfan_orchestrator import region_mask

    assert region_hint.is_placeholder_example is newfan_schemas.is_placeholder_example
    assert region_mask.is_placeholder_example is newfan_schemas.is_placeholder_example
    assert region_hint.is_placeholder_example("自社名(ロゴや社判も登録できます)") is True
    assert region_hint.is_placeholder_example("株式会社千曲川ホーム") is False


def test_placeholder_example_で落ちてヒントは渡らない(monkeypatch) -> None:
    """sample2 ← sample13: 記入例の帳票から作ったテンプレートは、別雛形に当てると候補
    （別の項目の値）が渡って従ってしまう。種類判定は unknown なので kind_conflict では
    落ちない。例示値の語彙で落とす。"""
    monkeypatch.setenv("REGION_KIE_HINTS", "1")
    schema = _field_schema("issuer_name", "string", "自社名(ロゴや社判も登録できます)")
    spans = [
        _sp(1, "請求日付令和02年01月31日", [320, 220, 480, 260]),
        _sp(2, "請求番号 GS0001", [320, 270, 480, 300]),
        _sp(3, "Pyxis", [320, 310, 480, 340]),
    ]
    out, report = llm_nodes.build_region_hints(schema, _PAGES_1, spans)
    assert report.dropped == {"issuer_name": "placeholder_example"}
    assert "region_hint" not in out["fields"][0]
    # 参考表示のために例示値と候補は detail に残る
    assert report.detail["issuer_name"]["example_value"] == "自社名(ロゴや社判も登録できます)"
    # 同じ帳票（候補が正しくても）でも落とす: 記入例語は値としての情報を持たない
    _, report2 = llm_nodes.build_region_hints(
        schema, _PAGES_1, [_sp(1, "自社名(ロゴや社判も登録できます)", [320, 220, 480, 260])]
    )
    assert report2.dropped == {"issuer_name": "placeholder_example"}


def test_placeholder_example_は候補0件_型_種類より先(monkeypatch) -> None:
    """記入例語はテンプレート側の欠陥なので、この帳票の事情（候補の有無・型・種類）より先に
    伝える。候補 0 件でも no_spans ではなく placeholder_example（「記入例ではない帳票で領域を
    引き直してください」と案内でき、metrics でもテンプレート単位に数えられる）。"""
    monkeypatch.setenv("REGION_KIE_HINTS", "1")
    schema = _field_schema("total_amount", "money_jpy", "金額")
    _, report = llm_nodes.build_region_hints(schema, _PAGES_1, [_OUT])
    assert report.dropped == {"total_amount": "placeholder_example"}
    _, report2 = llm_nodes.build_region_hints(schema, _PAGES_1, [_sp(1, "備考", [320, 220, 480, 260])])
    assert report2.dropped == {"total_amount": "placeholder_example"}
    # 記入例語でなければ従来どおり
    schema3 = _field_schema("total_amount", "money_jpy", "395,217")
    _, report3 = llm_nodes.build_region_hints(schema3, _PAGES_1, [_OUT])
    assert report3.dropped == {"total_amount": "no_spans_in_region"}


def test_placeholder_example_改行入りの例示値も_build_region_hints_経由で落ちる(monkeypatch) -> None:
    """本番は sanitize_example_value（改行・制御文字を**削除**）を通った後に判定する。
    「Lorem\nipsum」は「Loremipsum」になるので、強い印は空白を任意にしてある。"""
    monkeypatch.setenv("REGION_KIE_HINTS", "1")
    schema = _field_schema("customer_name", "string", "Lorem\nipsum")
    _, report = llm_nodes.build_region_hints(schema, _PAGES_1, [_sp(1, "大熊邸", [320, 220, 480, 260])])
    assert report.dropped == {"customer_name": "placeholder_example"}
