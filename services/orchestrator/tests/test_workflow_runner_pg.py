...
def test_load_extract_resultは実DDLからoriginal_nameとfieldsを返す() -> None:
    ...
    try:
        ...
        # pages / run_spans が無い段階（0008 より前の run と同じ観測）: 表題部の材料は空
        ...
        assert got["page1_spans"] == []
        assert got["page1_height"] is None

        # 1 ページ目の寸法と run_spans が揃えば、classify ゲートの表題部信号（ADR-0008）
        # の材料が返る。2 ページ目の span は混ざらない。
        spans_p1 = [
        ...
        got = store.load_extract_result(tenant, run_id)
        assert got["page1_spans"] == spans_p1
        assert got["page1_height"] == 1400
    finally:
        with owner.begin() as c:
