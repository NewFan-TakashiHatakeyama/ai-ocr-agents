          ...
          <aside className="rgn-right">
            {loading ? (
              ...
              <>
                ...
                {/* 読取領域が現在どこまで効くのかを正直に書く。以前はここで
                    「レイアウト違いを検知できないので読取領域を確定することを
                    おすすめします」と勧めていたが、検知結果が人に届く経路は
                    既定で閉じている（orchestrator nodes.py の guard_enforced()
                    が既定 off、web も metrics.region.mismatch_fields を読まない）。
                    効かないものを勧めるのは、機能が未完成であることより害が大きい */}
                {(drawMode === "include" || includes.length > 0) && (
                  <p className="sub" style={{ margin: 0 }} role="note">
                    読取領域は<b>いまのところ抽出結果を変えません</b>。指定しておくと、
                    項目が想定と違う位置で見つかったことの検知に使われますが、その結果は
                    現在ログと集計にしか出ません。
                  </p>
                )}
