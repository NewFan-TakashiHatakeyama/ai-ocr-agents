...
| **5** | ガード有効化: shadow 実測に基づき許容パラメータ（5% / 50% / 過半 / **`MIN_FIELDS_FOR_LAYOUT`**）を確定し `REGION_GUARD_ENFORCE` を on | 2 の実測後 | 誤 mismatch 率が許容水準（doc レベル判定込みで実測決定）。**n≤2 のスキーマで per-field レビューが出ること**（C33） |<br>**【2026-09-06 実測済み・有効化しない】** `golden/src/newfan_golden/region_guard_shadow.py` で「正しい領域（誤検知の測定）」「全項目ずれ（別レイアウト判定）」「1 項目だけずれ（per-field レビューが増える経路）」の 3 アームを回した。機構は意図どおり動くが、**正しい領域が機械的に完璧**（実検出位置から生成）なので実利用者が手で引いた矩形での誤検知率は測れていない。文書 2 件では許容パラメータを決められないため`REGION_GUARD_ENFORCE` は既定 off のまま。実運用で領域付き帳票が貯まってから `metrics.region.mismatch_fields` を集計して決める。
