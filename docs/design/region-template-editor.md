...
| D12 | プレビュー UI は既存ダイアログを全画面 `.tpl-overlay` に拡張（クラス名維持必須 — `web/app/documents/[id]/page.tsx:131-137` のショートカットガードが依存）。v1 の矩形操作は「描く・選ぶ・消す・ゴースト確定・モード切替」のみ。**移動・四隅リサイズハンドルは v1 から削除**（再ドラッグ置換で代替）。キャンバスは**ページ全体 fit（contain）表示**でスクロール問題を消す。ズームは非ゴール。 **【2026-09-24】** 幕は `createPortal` で `document.body` に描く。検証画面のヘッダ（`.rv-head`）は `backdrop-filter` を持ち、これは `position: fixed` の包含ブロックになるため、ヘッダの「✎ 領域・項目を編集」から開くと幕がヘッダ（高さ約 57px）に閉じ込められ、本体の上 400px ほど（ページ切替・ズーム・読取／除外の切替）が画面外に出ていた。クラス名は維持（ショートカット抑止は `document.querySelector` で見るのでポータルでも効く）。 |
...
- **v1 の対処（可視化のみ）**:
  ...
  3. **ワークフロー一覧の「⚠ 旧版スキーマ」バッジ【2026-09-24 追加】**: 1・2 はどちらも「その場」でしか出ない（保存直後のトースト、エディタでの保存・lint・有効化）ため、トーストを見落とすと後から一覧を見ても旧版参照のワークフローが分からなかった。`GET /v1/workflows` の各行に `stale_schema_refs: [{node_id, doc_type, schema_id, schema_version, latest_schema_id, latest_version}]` を載せ、web の一覧（`web/app/workflows/page.tsx`）は 1 件以上あればバッジ「⚠ 旧版スキーマ」を出し、title に内訳（`x1: invoice v2 → 最新 v4`）と直し方（スキーマを選び直す／「帳票種別（常に最新版を使う）」に切り替える）を出す。表示の導出は `web/lib/workflowStale.ts`。
     - **判定は増やさない**: 旧版かどうかは `workflows_repo.stale_schema_refs`（extract ノードが `schema_id` 指定で、当該 doc_type の最新版でない。doc_type 指定・存在しない id は対象外 ── L012 と同じ）の 1 箇所に置き、stale-workflows もこれを通すように改めた。版番号と最新版は `AdminRepository.schema_versions(tenant, ids)`（InMemory / Pg 両実装）で引く。Pg の「最新版」の定義（version 最大・is_active は見ない）は `db._LATEST_SCHEMA_LATERAL` として lint L012 の `PgWorkflowsRepository.schema_is_latest` と共有し、両者の判定が一致することを実 Pg のテストで固定した。
     - **N+1 にしない**: 一覧は全行の `schema_id` を集めて `schema_versions` を **1 回だけ**呼ぶ（`ANY(:ids)` + LATERAL の 1 SELECT）。Pg の SQL 文数はワークフロー数によらず一定（一覧 1 + 版の解決 1。各々 RLS の `set_config` を含めて 4 文）で、実 Pg の結合テストで 1 件と 5 件の文数一致を固定した。stale-workflows も旧版ごとの `get_schema_by_id` をやめて同じ 1 回に寄せた。
     - **status は問わない**: stale-workflows（保存後トースト）は「いま旧版で動いている」ことの警告なので active だけを返すが、一覧のバッジは draft / paused にも出す。有効化すれば旧版で動き、L012 は有効化（またはエディタでの保存・lint）の時点まで出ないため、一覧で先に知らせる（status はバッジの隣に出ている）。
     - **stale-workflows の応答は版一覧のスナップショットに揃える**（レビュー所見・回帰）: stale-workflows は版一覧（`schema_ids_for_doc_type`）で `latest_schema_id = ids[-1]` と旧版 `ids[:-1]` を決め、版番号は `schema_versions` で引く。両者は別の読み（Pg では別トランザクション）なので、その間に新版が保存されると `schema_versions` では `ids[-1]` も「最新でない」に見え、共通判定 `stale_schema_refs` をそのまま使うと**「最新として報告した版」を旧版参照として返す**（旧実装は `stale_set = set(ids[:-1])` で絞っていたので起きなかった）。`stale_schema_refs` の結果を `ids[:-1]` の集合で絞って元の性質に戻した（`latest_*` を versions 側から作る案は採らない ── 候補のワークフローは `ids[:-1]` で引いているので、`ids[-1]` だけを固定しているワークフローが漏れ、別の形で矛盾する）。読みの間の新版は次の保存・次の呼び出しで拾う。`test_schema_stale_workflows.py` に、版一覧を読んだ直後に v5 を差し込む再現テストを置いた。一覧（`GET /v1/workflows`）は版と最新版を 1 つの SELECT で同時に引くのでこの食い違いは起きない。
     - **web のキャッシュ**: バッジは `["workflows"]` のクエリキャッシュ（staleTime 10 秒・フォーカス時の再取得なし）から出るので、旧版化・解消の操作の後に捨てる。キーと捨て方は `web/lib/workflowListCache.ts`（`WORKFLOWS_QUERY_KEY` / `invalidateWorkflowList`）の 1 箇所に置き、一覧画面と RunWorkflow（同じキーで一覧を持つ）が共有する。呼ぶのはスキーマの新版保存（テンプレート化・領域編集 = `useSchemaSaved`、スキーマ管理画面の保存）と、エディタの保存・有効化・停止。エディタが従来捨てていた `["workflow", id]` は `["workflows"]` と prefix が違い一覧に届かない（当初はここが抜けていて、エディタで直して一覧へ戻ってもバッジが残った。レビュー所見）。
     - 単体取得（`GET /v1/workflows/{id}`・`WorkflowDto`）には載せない（エディタは保存・lint のたびに L012 で同じことを出す）。web の型は `stale_schema_refs?` の任意（旧い gateway と組み合わせてもバッジが出ないだけで壊れない）。
     - 既知の差: InMemory の `WorkflowsRepository.schema_is_latest` は版を持たず「存在すれば最新」を返す（L012 は Pg でしか出ない。従来どおり）。バッジと stale-workflows は admin 側の版情報を使うので InMemory（dev の DATABASE_URL 無し）でも出る。
- **v2 (b) を実装（2026-09-13）**: `ExtractConfig` に `doc_type` を追加し、`schema_id` と**どちらか一方**を指定する。`doc_type` 指定は**実行のたびに最新版へ解決**する（orchestrator `resolve_schema_id_for_doc_type`: `is_active` かつ `ORDER BY version DESC LIMIT 1`。解決できなければ空スキーマで「成功」させず理由の分かる失敗）。lint は L013（種別が未登録・アーカイブ済みなら error）を追加し、doc_type 指定は L009 / L012 の対象外。種別のアーカイブは doc_type 指定の有効ワークフローも参照として拒む。実際に使った版は `node_outputs[x].schema_id` に残す。エディタは種別のドロップダウン（`GET /doc-types`）。(a) の repoint は採らない（有効化済みワークフローの定義を管理者の操作なしに書き換えるため）。
...
**【2026-09-13 実装】** フラグ `REGION_EXCLUDE_SKIP_ON_LAYOUT_MISMATCH`（既定 off）を実装した。ただし判定は位置ガード（KIE の**後**）ではなく、**KIE の前に決定論で測る `layout_probe`** を使う（除外は `structure_ocr` で掛けるため、順序が逆な位置ガードの結果は使えない）: 読取領域（include）の `example_value`（テンプレート元でその位置にあった値）を画素へ射影した領域内の span に `example_present`（norm_key 照合）で探し、**例示値つき領域が 1 つ以上あるのに 1 つも見つからないページ**を別レイアウトとみなして除外を見送る。見送ったページは `metrics.region.skipped_exclude_pages`、判定材料は `layout_probe_matched / layout_probe_total` に残し、検証画面に「除外を適用せず N ページ（別レイアウトの可能性）」のバッジを出す。例示値つきの読取領域が無いスキーマでは従来どおり適用する（材料が無いので判定しない）。`structure_ocr` と `vl_fallback` の両方に同じ判定を掛ける。
...
| **`GET /v1/schemas/{doc_type}/stale-workflows`（admin・新設）** | 当該 doc_type の**旧版**（最新版以外の全版）の id を `process.extract.config.schema_id` に持つ **active** ワークフローを `{items:[{id,name,status,version,schema_id,schema_version}], latest_schema_id, latest_version}` で返す。web は保存成功後にこれを呼んで警告する（§4.4b）。lint L012 は有効化時にしか評価されないため、既に有効なワークフローにはこれが唯一の警告経路（第 3 回敵対的レビュー 2）。**【2026-09-24】** 旧版の判定は一覧のバッジと共通の `stale_schema_refs`、版番号は `admin.schema_versions` の 1 回に寄せた（応答は不変）。版一覧と版番号の読みの間に新版が保存されても、`items` は版一覧の旧版（`ids[:-1]`）に絞り、`latest_schema_id` に出した版を旧版参照として返さない（§4.4b-3） |
| **`GET /v1/workflows`（admin）【2026-09-24】** | 各行（`WorkflowListItemDto`）に `stale_schema_refs: [{node_id, doc_type, schema_id, schema_version, latest_schema_id, latest_version}]`（既定 `[]`）を追加。一覧の「⚠ 旧版スキーマ」バッジ用（§4.4b-3）。判定は stale-workflows と共通で、status は問わない。全行の schema_id をまとめて `admin.schema_versions` を 1 回だけ呼ぶ（N+1 にしない）。単体取得 `GET /v1/workflows/{id}` には載せない |
...
| `services/gateway/tests/test_workflow_list_stale.py`（新規・2026-09-24） | 一覧の `stale_schema_refs`（§4.4b-3）: 旧版ノードの内訳（node_id / doc_type / 参照版 / 最新版）/ 最新版・doc_type 指定・存在しない id は載らない / 複数ノード・別種別の混在で旧版のものだけ / status を問わない（draft も出す）かつ active の範囲で stale-workflows と一致 / 新版保存で旧版に変わる / `schema_versions` はワークフロー数によらず 1 回（重複 id は畳む）/ 単体取得には載せない |
| `web/lib/workflowStale.test.ts`（新規・2026-09-24） | バッジを出す条件（`stale_schema_refs` が空・未定義なら出さない）、title の内訳行（`x1: invoice v2 → 最新 v4`）と直し方の文言 |
| `services/gateway/tests/test_schema_stale_workflows.py`（追補・2026-09-24） | **版一覧と版番号の読みの間に新版が保存されても応答が自己矛盾しない**（`schema_ids_for_doc_type` の直後に v5 を差し込み、`latest_schema_id` は版一覧の最新 v4 のまま、`items` に v4 を旧版として載せない。§4.4b-3） |
| `web/lib/workflowListCache.test.ts`（新規・2026-09-24） | `invalidateWorkflowList` が staleTime 内の `["workflows"]` を無効化すること、エディタの `["workflow", id]` の無効化は一覧に届かない（だから別に呼ぶ）こと |
...
- **`services/gateway/tests/test_pg_workflow_list_stale_integration.py`（新規・2026-09-24）**: `PgAdminRepository.schema_versions` の実 SQL（LATERAL の最新版・`ANY(:ids)`・存在しない id と他テナントは載らない）と、lint L012 の `schema_is_latest` との判定一致。Pg リポジトリで組んだ app に `GET /v1/workflows` を投げ、旧版バッジの内訳と、**SQL 文数がワークフロー 1 件と 5 件で同じ**（field_schemas を引く文は 1 つ）ことを `before_cursor_execute` で数えて固定する（N+1 の再発防止線）。tenant は**テストごとに作って消す**（fixture `env`）。`field_schemas` は `UNIQUE (tenant_id, doc_type, version)` なので、固定 tenant では版 id を一意にしても (invoice, 1) 等が中断した実行の残骸や同時実行と衝突した（レビュー所見）。
...
| **4** | KIE ヒント: `_schema_for_prompt` の region_px 注入 / `_spans_for_prompt` の条件付き bbox / kie_extract.yaml 追記 / **markdown の部分除去方式（除外 span の text を行単位で除去し、残存時のみページ skip）の採否判断**（C24） | 1 の後、いつでも。**fixture 精度計測（タイトル等の位置特定型フィールドを含む）で改善が確認できた場合のみ出荷** | 計測レポートと region 無しスキーマのプロンプト完全一致の両立。**markdown 有効化時の精度影響も同計測に含める** |<br>**【2026-09-06 実測済み・出荷しない】** `docs/design/region-measurement-2026-09-06.md`。実スタックで region 有無の A/B（交互・各 5 試行）。全体 exact match は対照 0.864 → 介入 0.780 で**改善を示せなかった**（会社名は改善方向、住所は悪化）。ゲート未通過のため `REGION_KIE_HINTS` は既定 off。実装と計測ツールは入れてある（`golden/src/newfan_golden/region_ab.py`）。**評価データが実質 1 文書しか無く解像度が足りない**のが最大の制約で、まず位置依存項目を含む帳票を増やすのが先。<br>**【2026-09-07 再計測・結論を訂正】** `docs/design/region-measurement-2026-09-07.md`。帳票 30 枚を受領して測り直した結果、**1 回目の「改善を示せなかった」は領域の作り方の誤り**だった（抽出 AI が返した bbox から領域を起こしていたため、AI が取り違えた項目では領域も間違っていた）。正解値の紙面上の位置から領域を起こし直すと、**S1（21 枚）で p = 0.007 の有意な改善**（customer_name +8 / issuer_name +6 / total_amount +4）。テンプレートを別の帳票へ転用する S2 も 5 枚中 4 枚で改善した。**それでも既定 off のまま**にする理由は変わった: 効くのは領域が正しいときだけで、**1 行ずれると正解が壊れる**（実機で customer_name 3/3 → 0/3、返った値は隣の行の「大熊邸」）。しかも領域はずれる（29 枚中、同じ雛形の相手がいるのは 5 枚だけ）。さらに位置ガードはこの失敗を検知できない（領域の**中**から取った値なので一致と見なす）。次にやるのは「ヒントを反証可能にする」こと ── テンプレート元でその位置にあった値も一緒に渡し、種類が違えば従わない余地を作る。<br>**【2026-09-12 第 3 回計測・既定 on】** 設計 `region-field-add-and-hint-v2.md`（反証可能なヒント: 矩形ではなく候補 span の原文と例示値を渡し、渡す前に型・種類で落とし、従った／捨てたを集合演算で判定）を実装して測り直した記録が `docs/design/region-measurement-2026-09-12.md`。3 アーム（S1 その帳票自身 21 × 5、S2 同じ雛形への転用 5 × 5、S3 別雛形のテンプレート 12 × 5）で **6 つの出荷ゲートをすべて通過**: S1 p = 0.014、S2 0.497 → 0.583（p = 0.001）、S3 は対照と完全一致（12 / 12、間違ったテンプレートを当てても害なし）、第 2 回で壊れた sample8 の customer_name は 5/5 `kind_conflict` で守った。`REGION_KIE_HINTS` は既定 on（キルスイッチ。`0` で止める）。計測中に分かって直したこと: ヒント説明文が無関係な項目の読み方まで変える（適用範囲を明示）、LLM 出力の欠陥 3 種で run が failed になる（耐性を追加）。残す問題: 住所は S1 で悪化（対照のみ 18 / 介入のみ 5）── 郵便番号・社屋名を値に含めるかの慣例の問題で、住所にヒントを渡さない版は別の揺れを生んだため採らず、住所の正規化仕様を次の課題にする。
| **5** | ガード有効化: shadow 実測に基づき許容パラメータ（5% / 50% / 過半 / **`MIN_FIELDS_FOR_LAYOUT`**）を確定し `REGION_GUARD_ENFORCE` を on | 2 の実測後 | 誤 mismatch 率が許容水準（doc レベル判定込みで実測決定）。**n≤2 のスキーマで per-field レビューが出ること**（C33） |<br>**【2026-09-06 実測済み・有効化しない】** `golden/src/newfan_golden/region_guard_shadow.py` で「正しい領域（誤検知の測定）」「全項目ずれ（別レイアウト判定）」「1 項目だけずれ（per-field レビューが増える経路）」の 3 アームを回した。機構は意図どおり動くが、**正しい領域が機械的に完璧**（実検出位置から生成）なので実利用者が手で引いた矩形での誤検知率は測れていない。文書 2 件では許容パラメータを決められないため`REGION_GUARD_ENFORCE` は既定 off のまま。実運用で領域付き帳票が貯まってから `metrics.region.mismatch_fields` を集計して決める。<br>**【2026-09-13 実測・有効化して良い水準】** S1 / S2 / S3 の 38 帳票で 3 アーム × 1 試行（`docs/design/region-measurement-2026-09-13-guard.md`）。正しい領域での誤検知は field 4 / 135（3.0%、うち 3 件は `total_amount` の別出現）、doc レベル `layout_mismatch` の誤判定 0 / 25。全項目ずらしは field 98〜100%・doc レベル 25 / 26（漏れは判定対象 2 項目で下限 3 未満）、1 項目ずらしは 24 / 26 run で検知、別雛形（S3）は 12 run 中 7 が doc レベル・4 が per-field で見える。許容パラメータは現行値（5% / 50% / 過半 / 下限 3）のままで良く、有効化は環境変数（compose `.env` の `REGION_GUARD_ENFORCE=1`、ECS は tfvars `region_guard_enforce = "1"`）だけで済む。コードの既定は off のまま、**まず dev / staging で on** にして「設定領域外の位置で検出」の件数を見る。 |
...
16. **ワークフロー固定版の自動追従（repoint / `ExtractConfig.doc_type` による最新版解決）**: 本件スコープ外（敵対的レビュー第2回 C5 / D17）。理由は「有効化済みワークフローの抽出挙動が、管理者の操作なしに別の版へ切り替わる」副作用が本設計の検証範囲を超えるため。v1 は**可視化**（保存トーストの旧版参照警告 + lint L012 warning）に留め、恒久策は別チケットで判断する。§4.4b。
    **【2026-09-13 反転】** (b) `ExtractConfig.doc_type` を実装した（§4.4b）。運用者が「常に最新版」を望むワークフローだけが明示的に選ぶ形なので、「管理者の知らないところで挙動が変わる」副作用は選択した側に閉じる。(a) repoint は引き続き採らない。
...
9. **ワークフローの版固定（D17 / §4.4b）**: v1 は可視化のみで、有効化済みワークフローは旧版の除外設定を使い続ける。運用者が警告と lint L012 を無視した場合「保存したのに印影が消えない」は残る。自動 repoint を入れるかは実運用での踏み方を見てから判断する。
   **【2026-09-13】** 恒久策として `ExtractConfig.doc_type`（実行時に最新版へ解決）を入れた。版固定（`schema_id`）のワークフローは従来どおり可視化（保存後トースト＝全旧版を検出・L012）に留まる。
   **【2026-09-24】** 可視化の 3 つ目として、ワークフロー一覧に「⚠ 旧版スキーマ」バッジ（title に ノード・doc_type・v参照→v最新 の内訳）を出す（§4.4b-3）。トーストは保存直後の一度きり、L012 はエディタで保存・lint・有効化したときだけなので、見落とした旧版参照を後から一覧で拾えるようにした。判定は stale-workflows / L012 と共通のまま（web は判定を持たない）で、自動 repoint をしない方針も変えない。残るのは「一覧を見ない運用者は気づかない」ことで、通知（sink.notify 等）で知らせるかは実運用での踏み方を見て判断する。
...
11. ~~**`documents.doc_type` はテンプレート化時に更新されない**~~ → **解決済み（2026-09-06、§10-18 参照）**。ただし**自動取込（S3 / GDrive / M365）経路には配線していない**: `workflow_store` の `INSERT INTO documents` は doc_type 列を持たず、埋めると `process.classify` の許可リスト照合が初めて効き始めて `on_unknown="halt"` のワークフローが正当な帳票で止まり得る。またトリガーが複数マッチしたとき documents 行 1 つにどの種別を入れるかが未定義。別チケット。
    **【2026-09-13 自動取込にも配線】** `register_ingested`（S3 / GDrive / M365 / Box 共通）が、発火したワークフローの extract ノードから宣言種別を決めて `documents.doc_type` に書く（`doc_type` 指定はその種別、`schema_id` 指定は版の doc_type）。**発火したワークフローが別々の種別を指すときは決めない**（NULL のまま）。classify ゲートは宣言種別が入ることで `on_unknown="halt"` の誤 halt が**減る**側に働く（宣言が無いと内容信号だけになり、信号無しで halt していた）。
