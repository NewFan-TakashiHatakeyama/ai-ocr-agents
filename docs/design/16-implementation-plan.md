...
| 疎通テスト webhook / s3（`gateway/conntest.py`） | webhook は本配信と**同じ署名・ヘッダ**（`newfan_netguard.webhook_sig` を export と共有）で `{"event":"test"}` を SSRF ガード経由で 1 回 POST、2xx で tested。s3 は sink と同じ `boto3.client("s3")` で HeadBucket。失敗は 422 + 利用者向け理由（内部例外の文言は出さない）。これが無く webhook/s3 は永久に untested で L010 が有効化を拒んでいた。UI は行ごとの「疎通テスト」（フォルダ監視系は「今すぐ同期」が兼ねる）。設計 v0.2 §10 |
