...
| gateway 接続管理 | GET/POST /v1/connections + POST /v1/connections/{id}/test（postgres: SELECT 1。成功で status='tested'）。config の秘密は**再帰スキャン+部分一致**で拒否、GET は再帰マスク。**secret_ref は `.../conn/<tenant_id>/` 名前空間を強制**（クロステナント秘密窃取をレビューで実証→遮断） |
| 疎通テスト webhook / s3（`gateway/conntest.py`） | webhook は本配信と**同じ署名・ヘッダ**（`newfan_netguard.webhook_sig` を export と共有）で `{"event":"test","text":…}` を SSRF ガード経由で 1 回 POST、2xx で tested（`text` は Slack 互換の通知先 = sink.notify 向け。無いと 400 no_text で永久に untested）。s3 は sink と同じ `boto3.client("s3")` で HeadBucket（5 秒・再試行なし）。失敗は 422 + 利用者向け理由（内部例外の文言は出さない。httpx.InvalidURL も 500 にしない）。disabled は 409 で断る（1 クリックで配信再開させない）。これが無く webhook/s3 は永久に untested で L010 が有効化を拒んでいた。UI は行ごとの「疎通テスト」（フォルダ監視系は「今すぐ同期」が兼ねる）。dev（DATABASE_URL 無し）の既定 InMemoryWorkflowsRepository も admin を見て L010 を解く。設計 v0.2 §10 |
