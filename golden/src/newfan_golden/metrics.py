...
def _norm(v: Optional[str]) -> str:
    ...
    **``newfan_schemas.textnorm.norm_key``（D18）には置き換えない。** あちらは
    表記揺れ（敬称・全角半角・桁区切り）を同一視する照合キーで、意味が違う。
    ここはリリースゲート（exact_match / critical_exact_match）の指標なので、
    「¥128,000」と「128000」を同一視した瞬間に正規化器の退行が見えなくなる。
    加えて採点側は標準ライブラリだけで動く約束（pyproject）で、newfan_schemas
    への依存を持ち込まない。"""
    return (v or "").strip()
